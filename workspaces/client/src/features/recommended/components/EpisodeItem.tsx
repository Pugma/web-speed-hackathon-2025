...
// シリーズタイトルのスケルトンコンポーネント
const SeriesTitleSkeleton = () => (
  <div className="h-[12px] w-2/3 bg-[#2a2a2a] animate-pulse rounded mb-[4px]"></div>
);

// プレミアムタグのスケルトンコンポーネント
const PremiumTagSkeleton = () => (
  <span className="absolute bottom-[8px] right-[4px] inline-flex items-center justify-center rounded-[4px] bg-[#2a2a2a] p-[4px] w-[60px] h-[18px] animate-pulse"></span>
);
...
export const EpisodeItem = ({ episode }: Props) => {
  ...
  // 初期フェッチでのちらつきを防ぐため、少し遅延させる
  useEffect(() => {
    // データがストアにキャッシュされているか確認
